...
namespace Spectrogram
{
    static class Scale
    {
        public static Bitmap Vertical(int width, Settings settings, int offsetHz = 0, int tickSize = 3, int pxPerPx = 1)
        {
            double tickHz = 1;
            int minSpacingPx = 50;
            ...
            while (true)
            {
                ...
                if (pxBetweenTicks >= minSpacingPx * pxPerPx)
                    break;
            }

            Bitmap bmp = new Bitmap(width, settings.Height / pxPerPx, PixelFormat.Format32bppPArgb);
            ...
            using (var sf = new StringFormat() { LineAlignment = StringAlignment.Center })
            {
                ...
                {
                    int y = settings.PixelY(freq) / pxPerPx;
